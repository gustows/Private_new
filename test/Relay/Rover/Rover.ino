...
byte localAddress = Rover;     // address of this device
...
void setup() {
 ...
 // while (!Serial);
  rover = ide;
  Serial.println("Rover  " + dcode_status(rover) );
...
void loop() {
  ...
  if (millis() - lastSendTime > interval) {
    sendMessage(Rover, ide ,GStation,lastsend);
    lastSendTime = millis();            // timestamp the message
    //interval = interval + 100;    // 2-3 seconds
    Serial.println("*");
  }
